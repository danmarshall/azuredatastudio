...
suite('TelemetryService', () => {
	...
	test('Disposing', sinonTestFn(function () {
		...
		const service = new TelemetryService({ appenders: [testAppender] }, new TestConfigurationService());
	...
	class JoinableTelemetryService extends TelemetryService {
		...
		override publicLogError2<E extends ClassifiedEvent<T> = never, T extends IGDPRProperty = never>(eventName: string, data?: StrictPropertyCheck<T, E>): Promise<any> {
